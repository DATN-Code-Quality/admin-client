...
import { convertNlocToScale, renderColorRatting } from '~/utils';

const NO_DATA = 'NO_DATA';

const NCLoc: React.FC<{
  conditionItem: {
  ...
  const { coverage: coverageVal } = useSelector(
    SonarqubeSelector.getFiterSonarqube
  );
  const setCovarage = useCallback(
    (val: number) => {
      dispatch(setFilter({ coverage: val }));
    },
    [dispatch]
  );
  ...
  const formatedData: {
    ...
    return values?.reverse()?.map((item, index) => {
      ...
      return {
        val: convertNlocToScale(item?.val),
        key: `${
          val2 === '*'
            ? `>= ${val1}`
            : val1 === '*'
          ...
          return (
            <div
              ...
              className={`search-navigator-facet  ${
                coverageVal === +valueItem.val ? 'filter-active' : ''
              } `}
              onClick={() => setCovarage(+valueItem.val)}
            >
              {valueItem.icon}
